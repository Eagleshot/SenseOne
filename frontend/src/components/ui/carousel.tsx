...
type CarouselArrowDirection = "prev" | "next";
  ...
  ({ direction, className, variant = "outline", size = "icon", ...props }, ref) => {
    ...
    return (
      <Button
        ...
        variant={variant}
        size={size}
        className={cn("absolute h-8 w-8 rounded-full", positionClass, className)}
        disabled={isPrev ? !canScrollPrev : !canScrollNext}
        onClick={isPrev ? scrollPrev : scrollNext}
        {...props}
      >
        <Icon className="h-4 w-4" />
        <span className="sr-only">{isPrev ? "Previous slide" : "Next slide"}</span>
      </Button>
    );
  },
);
