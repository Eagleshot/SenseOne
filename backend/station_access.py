...
def require_station_exists(station_id: str) -> None:
    ...
    if not station_repo.station_exists(station_id):
...
def require_station_view(station_id: str, user) -> None:
    ...
    if not station_repo.can_view(station_id, user):
        raise HTTPException(
...
def can_edit_station(station_id: str, user) -> bool:
    ...
    return station_repo.owner_or_admin(station_repo.station_owner_id(station_id), user)
...
def require_station_edit(station_id: str, user) -> None:
    ...
    """
    owner_id = station_repo.station_owner_id(station_id)
    if owner_id is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Unknown station id.")
    if station_repo.owner_or_admin(owner_id, user):
        return
    if not station_repo.can_view(station_id, user):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Unknown station id.")
    raise HTTPException(
